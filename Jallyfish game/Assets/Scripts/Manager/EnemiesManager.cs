...
{
    public event Action<EnemyHealthScript> BossInstantiated;
    ...
    public void InstantiateEnemies(SaverData saverData)
    {
        for (int i = 0; i < saverData.EnamyType.Count; i++)
        {
            ...
            EnemyHealthScript enemyHealthScript = InstantiateEnemies(saverData.EnamyType[i], positionShark);
    ...
    public void InstantiateEnemies(List<EnemySpawner> enemySpawners)
    {
        foreach (EnemySpawner spawner in enemySpawners)
        {
            for (int i = 0; i < spawner.enemyCount; i++)
            {
                ...
                InstantiateEnemies(ESharkType.Shark, spawningPoint);
            }

            for (int i = 0; i < spawner.enemyCountLittle; i++)
            {
                Vector3 spawningPoint = spawner.GetSpawningPoint();
                InstantiateEnemies(ESharkType.LittleShark, spawningPoint);
            }

            for (int i = 0; i < spawner.enemyCountTerrorist; i++)
            {
                Vector3 spawningPoint = spawner.GetSpawningPoint();
                InstantiateEnemies(ESharkType.TerroristShark, spawningPoint);
            }

            for (int i = 0; i < spawner.enemyCountBoss; i++)
            {
                Vector3 spawningPoint = spawner.GetSpawningPoint();
                EnemyHealthScript boss = InstantiateEnemies(ESharkType.FirstBoss, spawningPoint);

                if (BossInstantiated != null)
                {
                    BossInstantiated(boss);
                }
            }
        }
    ...
    private EnemyHealthScript InstantiateEnemies(ESharkType enemyType, Vector3 spawnPosition)
    {
        ...
        enemyAttackScript.Init(ResourceManager, AudioManager);
