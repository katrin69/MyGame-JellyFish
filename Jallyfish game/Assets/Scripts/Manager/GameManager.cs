...
public class GameManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        GateController.LoadSceneRequest += GateController_LoadSceneRequest;
    }

    private void GateController_LoadSceneRequest(EScene nextScene)
    {
        SceneLoadingManager.LoadScene(nextScene);
    }
    ...
    private void Start() //действия при старте игры
    {
        if (SpawnEnemies) //если акулы значит у нас тут появляеются то 
        {
            ...
            EnemyInstantiationManager.BossInstantiated += EnemyInstantiationManager_BossInstantiated;
    ...
    private void EnemyInstantiationManager_BossInstantiated(EnemyHealthScript obj)
    {
        Debug.Log("BOSS HEALTH SCRIPT: " + obj);
        GateController.SetTargetHealthScript(obj);
    }
